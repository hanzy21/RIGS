...
class OccupancyLoss(BaseLoss):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if empty_label != 0 or num_classes != 3:
            raise ValueError(
                "RIGS occupancy loss requires empty_label=0 and num_classes=3"
            )
        self.empty_label = empty_label
        self.num_classes = num_classes
        self.classes = list(range(num_classes))
        if voxel_sample_rates is not None:
            self.voxel_sample_rates = torch.tensor(voxel_sample_rates, dtype=torch.float32)
            print(f"[OccupancyLoss] num_classes={num_classes}, voxel_sample_rates={self.voxel_sample_rates.tolist()}")
        else:
            self.voxel_sample_rates = None
        ...
        if balance_cls_weight:
            if manual_class_weight is not None:
                self.class_weights = torch.tensor(manual_class_weight, dtype=torch.float32)
            else:
            ...
            print(self.__class__, f'class_weights ({num_classes}-class):', self.class_weights)
        else:
    ...
    def loss_voxel(self, pred_occ, sampled_xyz, sampled_label, occ_mask=None, metas=None):
        if sampled_label.numel() and (sampled_label.min() < 0 or sampled_label.max() > 2):
            raise ValueError("OccupancyLoss requires labels 0, 1, 2")
        if occ_mask is not None and not bool(occ_mask.all()):
            raise ValueError("RIGS training uses the complete grid; masked occupancy loss is disabled")
        ...
        # 保存原始的sampled_xyz，以便在应用occ_mask后同步过滤
        sampled_xyz_filtered = sampled_xyz
        if occ_mask is not None:
            occ_mask = occ_mask.flatten(1)
            sampled_label = sampled_label[occ_mask][None]
            # 同步过滤sampled_xyz，确保维度匹配
            # sampled_xyz的形状通常是 [B, N, 3]，occ_mask的形状是 [B, N]
            # 需要确保sampled_xyz和sampled_label的样本数量一致
            if len(sampled_xyz.shape) == 3:
                # sampled_xyz: [B, N, 3]
                ...
                if len(occ_mask.shape) == 2:
                    ...
                    if B == 1:
                        # 通常batch_size=1，直接过滤
                        sampled_xyz_filtered = sampled_xyz[0][occ_mask[0]].unsqueeze(0)  # [1, N_filtered, 3]
                    else:
                        # batch_size>1的情况，需要为每个batch分别过滤
                        sampled_xyz_list = []
                        for b in range(B):
                            mask_b = occ_mask[b]  # [N]
                            sampled_xyz_list.append(sampled_xyz[b][mask_b])  # [N_filtered_b, 3]
                        # 由于每个batch的N_filtered可能不同，不能直接stack
                        # 这里假设所有batch的mask长度相同（通常情况）
                        # 如果不同，会报错，但这种情况很少见
                        sampled_xyz_filtered = torch.stack(sampled_xyz_list, dim=0)  # [B, N_filtered, 3]
                elif len(occ_mask.shape) == 1:
                    # occ_mask: [N]，假设batch_size=1
                    sampled_xyz_filtered = sampled_xyz[0][occ_mask].unsqueeze(0)  # [1, N_filtered, 3]
                else:
                    # Fallback: 假设batch_size=1
                    sampled_xyz_filtered = sampled_xyz[0][occ_mask.flatten()].unsqueeze(0)  # [1, N_filtered, 3]
            elif len(sampled_xyz.shape) == 2:
                # sampled_xyz: [N, 3]
                if len(occ_mask.shape) == 1:
                    sampled_xyz_filtered = sampled_xyz[occ_mask]  # [N_filtered, 3]
                elif len(occ_mask.shape) == 2 and occ_mask.shape[0] == 1:
                    sampled_xyz_filtered = sampled_xyz[occ_mask[0]]  # [N_filtered, 3]
                else:
                    sampled_xyz_filtered = sampled_xyz[occ_mask.flatten()]  # [N_filtered, 3]

        # Per-class voxel subsampling
        subsample_mask = None
        if self.voxel_sample_rates is not None and sampled_label.numel() > 0:
            flat_label = sampled_label.flatten()
            rand_vals = torch.rand_like(flat_label, dtype=torch.float32)
            rates = self.voxel_sample_rates.to(flat_label.device)
            keep_prob = rates[flat_label.clamp(0, len(rates) - 1)]
            subsample_mask = rand_vals < keep_prob
        ...
        for semantics in pred_occ:
            ...
            if subsample_mask is not None:
                sem_sub = semantics[:, :, subsample_mask]
                lab_sub = sampled_label[:, subsample_mask] if sampled_label.dim() == 2 else sampled_label[subsample_mask].unsqueeze(0)
                xyz_for_loss = sampled_xyz_filtered[:, subsample_mask, :] if sampled_xyz_filtered.dim() == 3 else sampled_xyz_filtered[subsample_mask]
            else:
                sem_sub = semantics
                lab_sub = sampled_label
                xyz_for_loss = sampled_xyz_filtered

            loss_dict = {}
            cw = self.class_weights.type_as(sem_sub)
            if self.use_focal_loss:
                loss_dict['loss_voxel_ce'] = self.loss_voxel_ce_weight * \
                    self.focal_loss(sem_sub, lab_sub, xyz_for_loss, cw, ignore_index=255)
            else:
                if self.lovasz_use_softmax:
                    loss_dict['loss_voxel_ce'] = self.loss_voxel_ce_weight * \
                        CE_ssc_loss(sem_sub, lab_sub, cw, ignore_index=255)
                else:
                    loss_dict['loss_voxel_ce'] = self.loss_voxel_ce_weight * CE_wo_softmax(
                        sem_sub, lab_sub, cw, ignore_index=255)
            if self.use_sem_geo_scal_loss:
                if self.lovasz_use_softmax:
                    scal_input = torch.softmax(sem_sub, dim=1)
                else:
                    scal_input = sem_sub
                loss_dict['loss_voxel_sem_scal'] = self.loss_voxel_sem_scal_weight * sem_scal_loss(scal_input.clone(), lab_sub, ignore_index=255)
                loss_dict['loss_voxel_geo_scal'] = self.loss_voxel_geo_scal_weight * geo_scal_loss(scal_input.clone(), lab_sub, ignore_index=255, non_empty_idx=self.empty_label)
            if self.use_lovasz_loss:
                if self.lovasz_use_softmax:
                    lovasz_input = torch.softmax(sem_sub, dim=1)
                else:
                    lovasz_input = sem_sub
                loss_dict['loss_voxel_lovasz'] = self.loss_voxel_lovasz_weight * lovasz_softmax(
                    lovasz_input.transpose(1, 2).flatten(0, 1), lab_sub.flatten(), ignore=self.lovasz_ignore)
            if self.use_dice_loss:
                loss_dict['loss_voxel_dice'] = self.dice_loss(sem_sub, lab_sub)
...
def softmax_focal_loss(pred,
                       ...
                       reduction='mean',
                       avg_factor=None):
    r"""Pure-PyTorch softmax focal loss (replaces mmcv CUDA kernel which has
    a backward shape bug when reduction='none' + cls_weight).

    Args:
        pred (torch.Tensor): Logits with shape (N, C).
        target (torch.Tensor): Integer labels with shape (N,).
        weight (torch.Tensor, optional): Per-sample weight.
        cls_weight (torch.Tensor, optional): Per-class weight of shape (C,).
        gamma (float): Focal exponent. Defaults to 2.0.
        alpha (float): Balanced alpha. Defaults to 0.25.
        reduction (str): 'mean' | 'sum' | 'none'. Defaults to 'mean'.
        avg_factor: Unused, kept for API compatibility.
    """
    import torch.nn.functional as F
    ...
    # gather per-sample log-prob and prob of the target class
    nll = -log_p.gather(1, target_long.unsqueeze(1)).squeeze(1)  # (N,)
    pt = p.gather(1, target_long.unsqueeze(1)).squeeze(1)        # (N,)

    focal_weight = alpha * ((1.0 - pt) ** gamma)
    loss = focal_weight * nll                     # (N,)

    if cls_weight is not None:
        loss = loss * cls_weight[target_long]
    if weight is not None:
